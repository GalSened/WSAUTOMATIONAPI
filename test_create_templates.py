...
class WesignApiCreateTemplateTests(unittest.TestCase):
    ...
    def test_create_template_duplicate_template_success(self):
        r = self.__api_create_template_duplicate_request("CreateTemplateDuplicateTemplate", 'fec7d1ad-18ff-46fa-83b8-08dac0825c40')
        assert r.status_code == StatusCode.OK
        response = r.json()
    ...
    def test_create_template_and_download_template_success(self):
        ...
        assert len(template) == 36, "Template not created"
        assert len(templatename) > 0
        download_req = self.__api_create_template_download_request(template)
    ...
    def test_delete_multi_documents_by_document_collection_id_success(self):
        ...
        for x in range(5):
            ...
            r = requests.post(self.settings['Base_Url'] + 'templates', data=json.dumps(req), headers=headers)
    ...
    def __api_create_template_download_request(self, template_id):
        headers = {'content-type': 'application/json', 'Authorization': 'Bearer ' + self.token}
